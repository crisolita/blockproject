...
export const sellNFT = async (req: Request, res: Response) => {
  try {
    ...
    if (nft && owner[0] === user?.wallet && user?.acctStpId && nft.precio_max) {
      ...
      if (
        !now.isBetween(
          moment(event?.fecha_inicio_venta),
          moment(event?.fecha_final_venta)
        )
      )
        return res.status(400).json({ error: "Ha finalizado la venta" });
      if (now.isAfter(nft.caducidadVenta) || !nft.marketplace)
        return res.status(400).json({ error: "NFT ha caducado para la venta" });
...
export const buyNFTfirstStep = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Ha finalizado o no ha empezado la venta" });
    ...
    precios = precios.sort(
      (
        a: { fecha_tope: string | number | Date },
        b: { fecha_tope: string | number | Date }
      ) => {
    ...
    if (adicionales && order.adicionalesIds) {
      ...
      const interseccion = adicionales.filter((x: number) =>
        adicByOrder.has(x)
      );

      for (let inter of interseccion) {
        const adicional = await prisma.adicionales.findUnique({
          where: { id: inter },
        });
    ...
    if (respuestas && order.preguntasIds) {
      ...
      for (let id of order.preguntasIds) {
        ...
        const exist = respuestas.find(
          (x: any) => x.pregunta === pregunta?.pregunta
        );
    ...
    if (codigo_descuento && order.codigo_descuento) {
      ...
      const cupon = await prisma.codigos_descuentos.findUnique({
        where: { cod: codigo_descuento },
      });

      if (
        order.codigo_descuento.includes(codigo_descuento) &&
        cupon &&
        cupon.veces_restantes > 0
      ) {
        priceActual = priceActual - (priceActual * cupon.porcentaje) / 100;
        await prisma.codigos_descuentos.update({
          where: { cod: codigo_descuento },
          data: { veces_restantes: cupon.veces_restantes - 1 },
        });
      }
    }
    ...
    if (buyer && buyer.wallet && seller?.acctStpId && priceActual) {
      ...
      return res
        .status(400)
        .json({ error: "Datos de comprador o vendedor faltantes" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json(error);
...
export const createAndSellNFT = async (req: Request, res: Response) => {
  try {
    ...
    if (event.creator_id !== user?.id)
      return res.status(400).json({ error: "user no ha creado el evento" });
    const now = moment();
    if (now.isAfter(moment(event?.fecha_final_venta)))
      return res.status(400).json({ error: "Ha finalizado la venta" });
    let orders = [];
    let nfts = [];

    if (
      moment(priceBatch[0].fecha_tope).isAfter(moment(event.fecha_inicio_venta))
    )
      return res
        .status(400)
        .json({ error: "Fecha tope es despues de fecha inicio de venta" });
    for (let precio of priceBatch) {
      if (!moment(precio.fecha_tope).isValid())
        return res
          .status(400)
          .json({ error: "Fecha invalida en precio batch" });
    }
    if (user && user.wallet && user.acctStpId) {
      ...
      if (codigo_descuento) {
        for (let codigo of codigo_descuento) {
          const exist = await prisma.codigos_descuentos.findUnique({
...
export const validarCodigo = async (req: Request, res: Response) => {
  try {
    ...
    let order = await getOrder(orderId, prisma);
    if (!order?.codigo_descuento.includes(codigo))
      return res
        .status(404)
        .json({ error: "Codigo no existe para esta orden" });
    let cod = await prisma.codigos_descuentos.findUnique({
      where: { cod: codigo },
    });
    if (!cod || cod.veces_restantes < 1)
      return res.status(400).json({ error: "Codigo no existe o ya fue usado" });
    return res.json({ cod });
  } catch (error) {
    console.log(error);
...
export const cancelBuy = async (req: Request, res: Response) => {
  try {
    ...
    if (buyer?.id != order?.buyerId)
      return res.status(400).json({ error: "User no es el comprador" });
    if (!order) return res.status(404).json({ error: "Orden no encontrada" });
    if (order.completedAt || order.status != "pago_pendiente")
      return res.status(400).json({ error: "Order esta completa" });
    order = await prisma.orders.update({
      where: { id: Number(order.id) },
      data: {
