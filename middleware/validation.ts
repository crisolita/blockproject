...
export const querySchemaCreateAndSellNFT = Joi.object({
  ...
  adicionales: Joi.array().items(
    Joi.object({ concepto: Joi.string(), valor: Joi.number().min(1) })
  ),
  caducidadVenta: Joi.string(),
  marketplaceSell: Joi.boolean(),
  license_required: Joi.number().min(0),
  codigo_descuento: Joi.array().items(
    Joi.object({
      veces_restantes: Joi.number().min(1),
      codigo: Joi.string(),
      porcentaje: Joi.number().max(100).min(1),
    })
  ),
...
export const querySchemaCreateEvent = Joi.object({
  ...
  modalidad: Joi.string()
    .valid("Triathlon", "Running", "Ciclismo", "Natacion")
    .required(),
  instagram: Joi.string(),
  twitter: Joi.string(),
  facebook: Joi.string(),
  distancia: Joi.number(),
  subcategoria: Joi.string().valid(
    "KM_5",
    "KM_10",
    "Media_maraton_21km",
    "Maraton_42km",
  ...
  fecha_inicio_venta: Joi.string(),
  fecha_final_venta: Joi.string(),
  fecha_asignacion: Joi.string(),
...
export const querySchemaEditEvent = Joi.object({
  ...
  place: Joi.string(),
  date: Joi.string(),
  modalidad: Joi.string().valid("Triathlon", "Running", "Ciclismo", "Natacion"),
  instagram: Joi.string(),
  twitter: Joi.string(),
  facebook: Joi.string(),
  distancia: Joi.number(),
...
export const querySchemaSetUserRol = Joi.object({
  ...
  user_rol: Joi.string().valid("ORGANIZADOR", "DEPORTISTA").required(),
...
export const querySchemaDorsal = Joi.object({
  dorsales: Joi.array().items(
    Joi.object({
      nftId: Joi.number().required(),
      dorsal: Joi.string().required(),
    })
  ),
});
...
export const querySchemaBuy = Joi.object({
  codigo_descuento: Joi.string(),
  orderId: Joi.number().required(),
  adicionales: Joi.array().items(Joi.number()),
  respuestas: Joi.array().items(
    Joi.object({ pregunta: Joi.string(), respuesta: Joi.string() })
  ),
});
